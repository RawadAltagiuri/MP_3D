...
function [] = softRobot_animation(sp, formattedPath)
    ...
    for k=1:1:steps
        ...
        % for i = 1:size(effectors, 1)
        %     plot3(effectors(i,1),effectors(i,2),effectors(i,3),'.','Color','b');
        % end
        ...
        if k ~= 1
            timeToPause = calculateCost(sp, formattedPath(:,:,k - 1), formattedPath(:,:,k));
            pause(timeToPause / speedMultiplier);
        end
    end
end
